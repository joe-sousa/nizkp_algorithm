...
/* Input P = (x1, y1, Z), Q = (x2, y2, Z)
   ...
   or P => P', Q => P + Q
*/
uECC_VLI_API void XYcZ_add(uECC_word_t * X1,
                     uECC_word_t * Y1,
                     uECC_word_t * X2,
                     uECC_word_t * Y2,
                     uECC_Curve curve) {
...
int scalar_mult(const uint8_t *point,
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) _private, scalar, num_bytes);
    bcopy((uint8_t *) _public, point, num_bytes*2);
#else
    uECC_vli_bytesToNative(_private, scalar, BITS_TO_BYTES(curve->num_n_bits));
    uECC_vli_bytesToNative(_public, point, num_bytes);
    uECC_vli_bytesToNative(_public + num_words, point + num_bytes, num_bytes);
#endif
    ...
    EccPoint_mult(_public, _public, _private, 0, curve->num_n_bits + 1, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
            uECC_vli_nativeToBytes(scalar, BITS_TO_BYTES(curve->num_n_bits), _private);
            uECC_vli_nativeToBytes(result, curve->num_bytes, _public);
            uECC_vli_nativeToBytes(
                result + curve->num_bytes, curve->num_bytes, _public + curve->num_words);
#endif
